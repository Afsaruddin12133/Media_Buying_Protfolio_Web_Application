          ...
          <div className="fixed inset-0 z-50 overflow-y-auto pointer-events-none">
            <div className="min-h-full flex items-center justify-center p-4 sm:p-6 lg:p-8">
              ...
              >
                ...
                <button
                  onClick={onClose}
                  className="absolute top-4 right-4 z-10 w-10 h-10 bg-white border border-black/10 text-[#000] flex items-center justify-center hover:bg-brandAccent hover:text-white hover:border-brandAccent transition-colors duration-300"
                ...
                {/* Hero Image Area */}
                <div className="w-full h-48 sm:h-64 relative bg-gray-100 shrink-0 border-b border-black/10">
                  <img
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-10">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black font-display text-[#000] tracking-tighter uppercase leading-none">
                      {study.title}
                    </h2>
                  </div>
                </div>
                ...
                {/* Scrollable Content */}
                <div className="overflow-y-auto p-6 md:p-10 space-y-12 bg-brandBg custom-scrollbar">
                  
                  {/* Executive Summary */}
                  <div>
                    <h3 className="text-xl font-display font-black text-[#000] mb-4 tracking-tighter uppercase">Executive Summary</h3>
                    <p className="text-brandMuted leading-relaxed font-medium">{study.description}</p>
                  </div>
                  ...
                  <div className="bg-white p-6 border border-black/10 shadow-[4px_4px_0px_var(--color-brandAccent)]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                      {study.metrics && Object.entries(study.metrics).map(([key, val]) => (
                        <div key={key} className="border-l-2 border-brandAccent pl-4">
                          ...
                          <p className="text-2xl font-display font-black text-[#000]">{val}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-brandMuted leading-relaxed font-medium">
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-brandMuted leading-relaxed font-medium">
                        {study.strategy || study.solution || study.details || "We implemented a full-funnel approach using advanced tracking and creative testing."}
                      </p>
                    </div>
                  </div>
                        ...
                        <ul className="space-y-3">
                          {study.results.map((r, i) => (
                            <li key={i} className="flex items-start gap-3">
                              ...
                              <span className="text-sm text-brandMuted font-medium">{r}</span>
                            </li>
                          ))}
                        </ul>
                      ) : (
